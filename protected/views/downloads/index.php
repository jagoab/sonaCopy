...
<div class="panel panel-default" style="">
	<div class="panel-heading">
        ...
        <?php
				...
				<img class="img-responsive"
					src="<?php echo Yii::app()->request->baseUrl . "/images/descargas/".str_replace(" ",'_',$guia['name']).".png"; ?>">
				<div class="estiloBrochureDownload">
					<a
						href="<?php echo Yii::app()->request->baseUrl . $guia['path']; ?>"> <?php echo $guia['name']; ?></a>
				</div>
			</div> 
                     <?php
										}
	...
	<div class="panel-body">
		<div class="row helvetica_neueregular">
			<div class="col-md-4">
				<label><?php echo Yii::t('forms', 'Files Type'); ?></label> <select
					id="file_type_select">
			...
			<div class="col-md-4">
				<label><?php echo Yii::t('forms', 'Product Type'); ?></label> <select
			...
			<div class="col-md-4">
				...
				<label><?php echo Yii::t('forms', 'Product'); ?></label> <select
					id="product_select">
					<option value="0" selected="selected"><?php echo Yii::t('forms', 'All'); ?></option> 
            <?php
												foreach ($filtros as $filtro)
												{
													foreach ($filtro->products as $prod)
													{
														?>              
                    <option
						class="product filter<?php echo $filtro['id']; ?>"
						value="<?php echo $prod['id']; ?>"><?php echo Yii::t('forms', $prod['name']); ?></option>       
                <?php
													}
												}
												?> 
...
<br />
<table class="download helvetica_neueregular" style="width: 100%;">
	<tr>
    ...
    <?php
					...
					$products = $lang = CHtml::listData(ProductFiles::model()->findAll('file=' . $File['fileid']), 'id', 'product');
    ...
    <tr class="todos" fileid="<?php echo $File['fileid']; ?>" language="<?php echo $language ?>" filter='<?php echo $File['type']; ?>'<?php foreach ($products as $product){echo 'product'.$product.'="1"';} ?> product_type='<?php echo $File['Product_Type_id']; ?>'  style="border-bottom: 1px solid #c5c5c5; <?php if($language!=Yii::app()->language) echo 'background-color: #f0f0f0';?>">
		...
		<th style="width: 150px;"><a
			href="<?php echo Yii::app()->request->baseUrl . $File['path']; ?>">

				<div style="margin-left: 60px; width: 32px; height: 35px;">
					<img src="<?php echo $image_file; ?>" class="img-responsive">
				</div>
		</a></th>
...
<div id="search_error" style="display: none;"
	class=" alert alert-danger text-center"><?php echo Yii::t('forms','No results found for your search'); ?></div>
<div id="select_filter" class=" alert alert-success"
	style="background-color:"><?php echo Yii::t('forms','Select a filter'); ?></div>
  <?php if(Yii::app()->language!='en'){?> <div class=" text-cnter "
	style="margin-top: 50px;">
	<span class="btn btn-primary" id="english_results">See English Results</span>
</div><?php }?>